const std = @import("std");

pub const grid = @import("grid");
pub const parse = @import("parse");
pub const math = @import("math");

pub const Half = struct {
    part: u2 = 0,
    testContents: ?[]const u8 = null,
    expectedResult: i64 = 0,
    actualResult: i64 = 0,
    runFn: TestFn,
    runTime: i128 = 0,

    pub fn init(alloc: std.mem.Allocator, day: u8, half: u1, testFn: TestFn) !Half {
        const infile = try std.fmt.allocPrint(alloc, "inputs/2024/{}.txt", .{day});
        defer alloc.free(infile);
        var file = try std.fs.cwd().openFile(infile, .{});
        defer file.close();

        return .{
            .part = half,
            .runFn = testFn,
            .testContents = try file.readToEndAlloc(alloc, 100_000),
        };
    }
    pub fn deinit(self: Half, alloc: std.mem.Allocator) void {
        if (self.testContents) |tc| alloc.free(tc);
    }

    pub fn run(self: *Half) !bool {
        const start = std.time.nanoTimestamp();
        self.actualResult = try self.runFn(self.testContents.?);
        self.runTime = std.time.nanoTimestamp() - start;
        const ret = self.actualResult == self.expectedResult;
        if (!ret) std.log.err("Part {} failed. Expected '{}' got '{d:0.2}'", .{
            self.part,
            self.expectedResult,
            self.actualResult,
        });
        std.debug.print(
            "Part {} took {d:0.3}ms\n",
            .{
                self.part + 1,
                @as(f32, @floatFromInt(self.runTime)) / std.time.ns_per_ms,
            },
        );
        return ret;
    }
};
const TestFn = *const fn ([]const u8) std.mem.Allocator.Error!i64;
fn day1Test(in: []const u8) std.mem.Allocator.Error!i64 {
    var ret: i64 = 0;
    for (in) |c| {
        ret += if (c == '(') 1 else if (c == ')') -1 else 0;
    }
    return ret;
}
fn day2Test(in: []const u8) std.mem.Allocator.Error!i64 {
    var ret: i64 = 0;
    const idx: usize = blk: for (in, 0..) |c, i| {
        ret += if (c == '(') 1 else if (c == ')') -1 else 0;
        if (ret == -1) break :blk i;
    } else 0;

    return @intCast(idx + 1);
}
test {
    var d = try Day.init(std.testing.allocator, 0, day1Test, day2Test);
    d.part1.expectedResult = 138;
    d.part2.expectedResult = 1771;
    defer d.deinit();
    try std.testing.expect(try d.run());
}

pub const Day = struct {
    number: u8,
    part1: Half,
    part2: Half,

    allocator: std.mem.Allocator,

    pub fn init(alloc: std.mem.Allocator, day: u8, part1: TestFn, part2: TestFn) !Day {
        return .{
            .number = day,
            .part1 = try Half.init(alloc, day, 0, part1),
            .part2 = try Half.init(alloc, day, 1, part2),
            .allocator = alloc,
        };
    }
    pub fn deinit(self: Day) void {
        self.part1.deinit(self.allocator);
        self.part2.deinit(self.allocator);
    }

    pub fn run(self: *Day) !bool {
        std.debug.print("Running Day {}\n", .{self.number});
        return try self.part1.run() and try self.part2.run();
    }
};

/// Format timing in human-readable units
pub fn formatTime(ns: u64) TimingResult {
    if (ns < 1_000) {
        return .{ .value = @as(f64, @floatFromInt(ns)), .unit = "ns" };
    } else if (ns < 1_000_000) {
        return .{ .value = @as(f64, @floatFromInt(ns)) / 1_000.0, .unit = "µs" };
    } else if (ns < 1_000_000_000) {
        return .{ .value = @as(f64, @floatFromInt(ns)) / 1_000_000.0, .unit = "ms" };
    } else {
        return .{ .value = @as(f64, @floatFromInt(ns)) / 1_000_000_000.0, .unit = "s" };
    }
}

pub const TimingResult = struct {
    value: f64,
    unit: []const u8,

    pub fn format(
        self: TimingResult,
        writer: *std.Io.Writer,
    ) std.Io.Writer.Error!void {
        try writer.print("{d:.3}{s}", .{ self.value, self.unit });
    }
};

/// Run a function and measure its execution time
pub fn timed(
    comptime T: type,
    func: anytype,
    args: anytype,
) !struct { result: T, time_ns: u64 } {
    var timer = try std.time.Timer.start();
    const start = timer.read();
    const result = try @call(.auto, func, args);
    const end = timer.read();
    return .{ .result = result, .time_ns = end - start };
}

/// Benchmark a function over multiple iterations
pub fn benchmark(
    comptime T: type,
    func: anytype,
    args: anytype,
    iterations: usize,
) !struct { result: T, avg_ns: u64, min_ns: u64, max_ns: u64 } {
    var min: u64 = std.math.maxInt(u64);
    var max: u64 = 0;
    var total: u64 = 0;
    var result: T = undefined;

    var timer = try std.time.Timer.start();
    var i: usize = 0;
    while (i < iterations) : (i += 1) {
        const start = timer.read();
        result = try @call(.auto, func, args);
        const end = timer.read();
        const elapsed = end - start;

        total += elapsed;
        if (elapsed < min) min = elapsed;
        if (elapsed > max) max = elapsed;
    }

    return .{
        .result = result,
        .avg_ns = total / iterations,
        .min_ns = min,
        .max_ns = max,
    };
}

/// Parse all integers from a string
pub fn parseInts(comptime T: type, allocator: std.mem.Allocator, input: []const u8) ![]T {
    var list = std.ArrayListUnmanaged(T){};
    errdefer list.deinit(allocator);

    var it = std.mem.tokenizeAny(u8, input, " ,\n\t");
    while (it.next()) |token| {
        if (std.fmt.parseInt(T, token, 10)) |num| {
            try list.append(allocator, num);
        } else |_| {
            continue;
        }
    }

    return try list.toOwnedSlice(allocator);
}

/// Split input into lines
pub fn lines(input: []const u8) std.mem.TokenIterator(u8, .scalar) {
    return std.mem.tokenizeScalar(u8, input, '\n');
}

/// Common grid directions
pub const Direction = enum {
    north,
    south,
    east,
    west,

    pub fn delta(self: Direction) struct { dx: i32, dy: i32 } {
        return switch (self) {
            .north => .{ .dx = 0, .dy = -1 },
            .south => .{ .dx = 0, .dy = 1 },
            .east => .{ .dx = 1, .dy = 0 },
            .west => .{ .dx = -1, .dy = 0 },
        };
    }
};

/// Manhattan distance
pub fn manhattan(x1: anytype, y1: anytype, x2: anytype, y2: anytype) @TypeOf(x1) {
    const dx = if (x2 > x1) x2 - x1 else x1 - x2;
    const dy = if (y2 > y1) y2 - y1 else y1 - y2;
    return dx + dy;
}
